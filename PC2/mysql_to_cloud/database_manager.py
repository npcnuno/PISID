...
class DatabaseManager:
    def __init__(self, graph, player_id, fresh_start, cloud_pool, local_pool, decision_maker, decision_condition, current_sound, sound_condition):
        self.graph = graph
    ...
    def _fetch_latest_game_id(self):
        ...
        try:
            with conn.cursor() as cursor:
                cursor.execute("SELECT MAX(idJogo) FROM MedicaoPassagem WHERE idJogo IN "
        ...
        if self.fresh_start:
            ...
            try:
                with conn.cursor(dictionary=True) as cursor:
                    ...
                    cursor.execute("SELECT hora FROM Sound ORDER BY hora DESC LIMIT 1")
                    result = cursor.fetchone()
